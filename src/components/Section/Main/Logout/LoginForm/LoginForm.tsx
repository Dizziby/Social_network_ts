    ...
    return (
        <form className={styles.loginForm} onSubmit={handleSubmit(onSubmit)}>
            <div>
                ...
                <input className={styles.formInput} placeholder={"Email"} defaultValue="" {...register("email", {required: true, pattern: /^([a-z0-9_-]+\.)*[a-z0-9_-]+@[a-z0-9_-]+(\.[a-z0-9_-]+)*\.[a-z]{2,6}$/})} />
                {errors.email ? <div className={styles.error}>This field is required</div> : <div className={styles.error}></div>}
                <input className={styles.formInput} placeholder={"Password"} type={"password"} defaultValue="" {...register("password", {required: true})} />
                {errors.password ? <div className={styles.error}>This field is required</div> : <div className={styles.error}></div>}
                <input className={styles.formCheckbox} type="checkbox" {...register("rememberMe")}/><span>Remember Me</span>
                {isStopSubmit ? <div className={styles.stopSubmit}>{messageStopSubmit}</div> : <div className={styles.stopSubmit}></div>}
